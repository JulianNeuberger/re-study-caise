...
class BaseImporter(abc.ABC):
    ...
    def _fix_ner_tags(self, tokens: typing.List[model.Token],
        ...
        for i, e in enumerate(entities):
            tags = [tokens[i].ner_tag for i in e.token_indices]
            if 'O' not in tags:
                continue
            ...
            for token_index, ner_tag in zip(e.token_indices, tags):
            ...
            if r.head.ner_tag == 'O':
                relations[i] = model.Relation(
                    head=model.Entity(
                        text=r.head.text,
                        ner_tag='UNKNOWN',
                        token_indices=r.head.token_indices
                    ),
                    tail=r.tail,
                    type=r.type
                )
            ...
            if r.tail.ner_tag == 'O':
                relations[i] = model.Relation(
                    ...
                    tail=model.Entity(
                        text=r.tail.text,
                        ner_tag='UNKNOWN',
                        token_indices=r.tail.token_indices
                    ),
                    type=r.type
                )
    ...
    def _build_tokens(doc: stanza_model.Document) -> typing.List[model.Token]:
        stanza_tokens: typing.List[stanza_model.Token] = [t for s in doc.sentences for t in s.tokens]
